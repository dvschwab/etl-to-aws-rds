...
def extract_delimited_field(file_reader, position):
    ...
    assert (isinstance(position, int), f'The position argument {position} is not an intger')
...
if __name__ == '__main__':
    ...
    # Make sure field position is not negative
    if args.position < 0:
        print("The field position cannot be negative.")
        exit()
